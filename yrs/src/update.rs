use crate::block::{
    ...
    BLOCK_SKIP_REF_NUMBER, HAS_ORIGIN, HAS_PARENT_SUB, HAS_RIGHT_ORIGIN, ClockType,
};
                            ...
                            if a.try_squash(b) {
                                ...
                                if block.is_item() {
                                    ...
                                    if diff > 0 {
                                        ...
                                        if let Some(new) = a.splice(diff as ClockType) {
        ...
        let remaining_blocks = if self.blocks.is_empty() {
            ...
            while let Some(mut block) = stack_head {
                ...
                if local_sv.contains(&id) {
                    let offset = local_sv.get(&id.client) as i64 - id.clock as i64;
                    ...
                    } else if offset == 0 || (offset as ClockType) < block.len() {
                        let offset = offset as ClockType;
    ...
    fn decode_block<D: Decoder>(id: ID, decoder: &mut D) -> Result<BlockCarrier, Error> {
        ...
        match info {
            BLOCK_SKIP_REF_NUMBER => {
                let len: ClockType = decoder.read_var()?;
                Ok(BlockCarrier::Skip(BlockRange { id, len }))
            }
            BLOCK_GC_REF_NUMBER => {
                let len: ClockType = decoder.read_len()?;
        ...
        for (client, blocks) in self.blocks.clients.iter() {
            ...
            while let Some(block) = curr {
                if block.is_skip() {
                    ...
                    e.0 = (remote_clock as i64 - block.id().clock as i64).max(0) as ClockType;
                    e.1.push(block);
                    curr = iter.next();
                    while let Some(block) = curr {
                        e.1.push(block);
        ...
        loop {
            ...
            if let Some(curr_write_block) = curr_write.as_mut() {
                ...
                if first_client != curr_write_block.id().client {
                    ...
                    let mut block_slice: Option<BlockCarrier> = None;
                    if diff > 0 {
                        if let BlockCarrier::Skip(skip) = curr_write_block {
                            // prefer to slice Skip because the other struct might contain more information
                            skip.len -= diff;
                        } else {
                            block_slice = Some(curr_block.splice(diff).unwrap());
                        }
                    }
...
impl Decode for Update {
    fn decode<D: Decoder>(decoder: &mut D) -> Result<Self, Error> {
        // read blocks
        let clients_len: ClockType = decoder.read_var()?;
        ...
        for _ in 0..clients_len {
            let blocks_len = decoder.read_var::<ClockType>()? as usize;

            let client = decoder.read_client()?;
            let mut clock: ClockType = decoder.read_var()?;
            let blocks = blocks
                .clients
                .entry(client)
...
impl BlockCarrier {
    pub(crate) fn splice(&self, offset: ClockType) -> Option<Self> {
        match self {
    ...
    pub(crate) fn len(&self) -> ClockType {
        match self {
    ...
    pub fn encode_with_offset<E: Encoder>(&self, encoder: &mut E, offset: ClockType) {
        match self {
            BlockCarrier::Block(x) => {
    ...
    pub fn integrate(&mut self, txn: &mut TransactionMut, offset: ClockType) -> bool {
        match self {
